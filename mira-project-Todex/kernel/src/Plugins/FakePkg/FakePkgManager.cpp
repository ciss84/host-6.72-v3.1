...
bool FakePkgManager::ShellCorePatch()
{
    ...
    if (s_Ret < 0)
    {
	  ...
	  s_Ret = Utilities::ProcessReadWriteMemory(s_Process, (void*)(s_TextStart + ssc_ext_hdd_patchA), 1, (void*)"\xEB", nullptr, true);
    if (s_Ret < 0)
    {
        WriteLog(LL_Error, "ssc_ext_hdd_patchA");
        return false;
    }
	  s_Ret = Utilities::ProcessReadWriteMemory(s_Process, (void*)(s_TextStart + ssc_ext_hdd_patchB), 1, (void*)"\xEB", nullptr, true);
    if (s_Ret < 0)
    {
        WriteLog(LL_Error, "ssc_ext_hdd_patchB");
        return false;
    }
 	  s_Ret = Utilities::ProcessReadWriteMemory(s_Process, (void*)(s_TextStart + ssc_disable_screenshot_patch), 5, (void*)"\x90\x90\x90\x90\x90", nullptr, true);
    if (s_Ret < 0)
    {
