...
struct dtrace_probe {
	...
	uint32_t dtpr_func_id;
	int dtpr_aframes;			/* artificial frames */
	dtrace_cacheid_t dtpr_predcache;	/* predicate cache ID */ /* 16B */
	dtrace_ecb_t *dtpr_ecb;			/* ECB list; see below */
	dtrace_ecb_t *dtpr_ecb_last;		/* last ECB in list */ /* 32B */
	void *dtpr_arg;				/* provider argument */
	dtrace_provider_t *dtpr_provider;	/* pointer to provider */ /* 48B */
	const char *dtpr_mod;				/* probe's module name */
	const char *dtpr_func;			/* probe's function name */ /* 64B */
	const char *dtpr_name;			/* probe's name */
	dtrace_probe_t *dtpr_nextmod;		/* next in module hash */ /* 80B */
	dtrace_probe_t *dtpr_prevmod;		/* previous in module hash */
	dtrace_probe_t *dtpr_nextfunc;		/* next in function hash */ /* 96B */
	dtrace_probe_t *dtpr_prevfunc;		/* previous in function hash */
	dtrace_probe_t *dtpr_nextname;		/* next in name hash */ /* 112B */
	dtrace_probe_t *dtpr_prevname;		/* previous in name hash */
	dtrace_genid_t dtpr_gen;		/* probe generation ID */ /* 128B */
#else
	dtrace_id_t dtpr_id;			/* probe identifier */
	dtrace_ecb_t *dtpr_ecb;			/* ECB list; see below */
	...
	const char *dtpr_mod;				/* probe's module name */
	const char *dtpr_func;			/* probe's function name */
	const char *dtpr_name;			/* probe's name */
...
typedef struct SceDtraceSymTbl {
	uint32_t symNum;
	SceDtraceSym *sym;
} SceDtraceSymTbl;

typedef struct SceDtraceStrTbl {
	uint32_t size;
	char *str;
} SceDtraceStrTbl;
...
struct dtrace_state {
	...
	uint32_t dts_nspeculations;			/* number of speculations */
...
#if defined(__ORBIS__)
	int dts_compress;
	int dts_host_version;
#define SCE_DTRACE_KERN_DTRACEP_MODE 1
	int dts_comm_mode;
	uint32_t dts_syminfo_num;
	struct SceDtraceSymInfo *dts_syminfo;
	uint32_t dts_strtbl_size;
	char *dts_strtbl;
#endif
};
...
#if defined(__ORBIS__)
extern register_t dtrace_getreg(struct trapframe *rp, uint_t reg);
#else
...
#if defined(__ORBIS__)
int sceDtraceOpenFromKern(void);
int sceDtraceCloseFromKern(void);
int sceDtraceGetDeviceStateFromKern(void **state);

typedef int (*SceDtraceIoctlFuncs)(void *, uint32_t *, void *hugeDataAddr, dtrace_state_t *);
int sceDtraceGetIoctlFuncs(SceDtraceIoctlFuncs **funcs, uint32_t *num);
#endif
