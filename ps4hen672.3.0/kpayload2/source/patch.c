...
PAYLOAD_CODE int shellcore_fpkg_patch(void)
{
	...
	if (ret)
	{
	 ...
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + ssc_sceKernelIsAssistMode_patchA), 5, "\x31\xC0\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + ssc_enable_vr_patch), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }  
